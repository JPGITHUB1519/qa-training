...
class Queue:
    ...
    def checkRep(self):
        ...
        if self.tail > self.head:
            assert (self.tail-self.head) == self.size #pragma: no cover
        if self.tail < self.head:
            assert (self.head-self.tail) == (self.max-self.size) #pragma: no cover
...
def test():
    ...
    # full
    assert q.full() == True
    # enqueque to a full queue
    assert q.enqueue(1) == False
    
    # dequeue Testing
    assert q.dequeue() == 1
    
    # dequeue upon not more
    q.dequeue()
    q.dequeue()
    assert q.head == 0
    # checkrep
